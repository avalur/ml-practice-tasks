...
class LoRALinear:
    ...
    * ``trainable_fraction()`` — the adapter's parameter count over the base's,
    ...
    def __init__(
        ...
        self.rank = rank
        self.alpha = alpha
        self.A = rng.standard_normal((rank, in_features)) / np.sqrt(in_features)
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        return x @ self.weight.T + (self.alpha / self.rank) * update
        # --- solution: end ---

    def merged_weight(self) -> np.ndarray:
        # --- solution: begin ---
        # `self.weight + …` builds a new array; `+=` here would quietly destroy
        # the base model everything else still shares.
        return self.weight + (self.alpha / self.rank) * (self.B @ self.A)
        # --- solution: end ---

    def trainable_fraction(self) -> float:
        # --- solution: begin ---
        return (self.A.size + self.B.size) / self.weight.size
        # --- solution: end ---
